...
def enter_new_recipe(recipe_name, meal_served, serving_size, recipe_ingredients,
    ...
    if recipe_name not in recipe_names:
        ...
        error = 'There is already a recipe with that name.'
    
# has user input ingredient info and adds that info to the recipe 
# storage file
        
def add_new_recipe(recipe_name, meal_served, serving_size, recipe_ingredients,
                   recipe_measurements, recipe_amounts):
    ingredients = convert_list_to_string(list_name=recipe_ingredients)
    measurements = convert_list_to_string(list_name=recipe_measurements)
    amounts = convert_list_to_string(list_name=recipe_amounts)
    add_recipes_to_file(recipe_name, meal_served, serving_size, ingredients, 
...
def add_recipes_to_file(recipe_name, meal_served, serving_size, ingredients, 
                        measurements, amounts):
